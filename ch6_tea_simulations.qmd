...
```{r}
hist(all_results$pvalue) #what does this histogram look like?
```

```{r}
sig <- sum(all_results$pvalue < 0.05)
sig

#proportion
sig/length(all_results$pvalue)
```
...
```{r}
sig <- sum(all_results_high_n$pvalue > 0.05)
sig

#proportion
sig/length(all_results_high_n$pvalue)
```
